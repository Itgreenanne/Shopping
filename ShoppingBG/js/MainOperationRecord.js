...
//起日不能晚於迄日的檢查
function NoLaterThanEndDateForRecord(dateString) {
...
//迄日不能早於起日的檢查
function NoEarlierThanStartDateForRecord(dateString) {
    endDate = dateString;
    ...
    else
    {
        switch (dataTypeIndex) {
            case 1:
                ...
                break;
            case 2:
                return UserDataParsedForAddAndDelete(beforeObj, afterObj);
                break;
            case 3:
                return ProductDataParsedForAddAndDelete(beforeObj, afterObj);
                break;
            case 4:
                return MemberDataParsedForAddAndDelete(beforeObj, afterObj);
                break;
        }
    }
}
...
function OperationDetailTableData(dataType, obj1, obj2) {
    ...
    return '<td class="beforeAndAfterColumn">' + before + '</td><td class="beforeAndAfterColumn">' + after + '</td>';
}
...
function DutyDataParsedForAddAndDelete(beforeObj, afterObj) {
    ...
    dataRow = OperationDetailTableData(1, beforeObj, afterObj);
    return dataRow;
}
...
function UserDataParsedForAddAndDelete(beforeObj, afterObj) {
...
function ProductDataParsedForAddAndDelete(beforeObj, afterObj) {
    if (beforeObj) {
        var jsonBeforeObj = JSON.parse(beforeObj);
        for (var key in jsonBeforeObj) {
            if (key == 'productDetail') {
...
//會員新增與刪改後欄位內容轉為中文
function MemberDataParsedForAddAndDelete(beforeObj, afterObj) {
    if (beforeObj) {
        var jsonBeforeObj = JSON.parse(beforeObj);
        for (var key in jsonBeforeObj) {
            if (key == 'gender') {
